...
impl Solution {
    ...
    pub fn find_median_sorted_arrays(mut nums1: Vec<i32>, mut nums2: Vec<i32>) -> f64 {
        ...
        // the constraint is that there must be equal numbers of two parts (the difference <= 1)
        // do binary split on both array, using a1 (split of a2 is based on constraint)
        // check if l1 < r2 and l2 < r1
        // this means max(l1, l2) < min(l2, l1),
        // if yes, we found a valid split
        // if no, we adjust the split
        // if l1 > r2, it means that the split needs to be at a lower place
        // if l2 > r1, it means that the split needs to be at a higher place
        ...
        if nums2.len() == 0 {
            if nums1.len() % 2 == 0 {
                return (nums1[nums1.len() / 2] as f64 + nums1[nums1.len() / 2 - 1] as f64) / 2.0;
            } else {
        ...
        let left_count = (total_len + 1) / 2;  // make the median at the left split



        let mut lbound = 0;
        let mut rbound = nums1.len();

        while lbound < rbound {
            let mut r1 = lbound + (rbound - lbound + 1) / 2;
            if left_count < r1 {
                // this split is not feasible
                // because the split for 2 will be < out of bound
                let l = nums1[left_count - 1];
                let r = i32::min(nums1[left_count], nums2[0]);
                if l <= r {
                    if total_len % 2 == 0 {
            ...
            let mut r2 = left_count - r1;
            if r2 > nums2.len() {
                // this split is not feasible
                // because the split for 2 will be > out of bound
                // [2], [1, 3, 4, 5, 6, 7, 8]
                let r = nums1[left_count - nums2.len()];
                let l = i32::max(nums1[left_count - nums2.len() - 1], nums2[nums2.len() - 1]);
                if l <= r {
                    if total_len % 2 == 0 {
                        return (l + r) as f64 / 2.0;
                    } else {
            ...
            // guard_1: split at very beginning of a1
            // guard_2: split at very end of a2
            let left_guard_1 = r1 == 0;
            let right_guard_2 = r2 == nums2.len();
            let left_guard_2 = r2 == 0;
            let right_guard_1 = r1 == nums1.len();

            let l1 = r1 + left_guard_1 as usize - 1;  // not to substract with overflow:
            let l2 = r2 + left_guard_2 as usize - 1;  // guard is true then value is discarded

            match (
                left_guard_1 || right_guard_2 || nums1[l1] <= nums2[r2],
                left_guard_2 || right_guard_1 || nums2[l2] <= nums1[r1]
            ) {
                (true, true) => {
                    let left_candidate = if left_guard_1 {
                        nums2[l2] as f64
                    } else if left_guard_2 {
                        nums1[l1] as f64
                    } else {
                        f64::max(nums1[l1] as f64, nums2[l2] as f64)
                    };
                    let right_candidate = if right_guard_1 {
                        nums2[r2] as f64
                    } else if right_guard_2 {
                        nums1[r1] as f64
                    } else {
                        f64::min(nums1[r1] as f64, nums2[r2] as f64)
                    };
                    if total_len % 2 == 0 {
                        return (left_candidate + right_candidate) / 2.0;
                    } else {
        ...
        // if we exit the loop, it means that the split is at the very beginning of a1 or the very end of a2
        if total_len % 2 == 0 {
            let r = nums1[0];
            let l = nums2[nums2.len() - 1];
            return (l + r) as f64 / 2.0;
        } else {
            return nums2[nums2.len() - 1] as f64;
        }
    }
}
...
mod test {
    ...
    fn test_q4() {
        let nums1 = vec![1, 2];
        let nums2 = vec![3, 4];
        let res = super::Solution::find_median_sorted_arrays(nums1, nums2);
        assert_eq!(res, 2.5);
    }
